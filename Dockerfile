# Multi-stage build for india-trai-regulation-mcp
...
ENV TRAI_DB_PATH=/app/data/trai.db

EXPOSE 9195

HEALTHCHECK --interval=30s --timeout=10s --start-period=10s --retries=3 \
  CMD node -e "fetch('http://localhost:9195/health').then(r=>r.ok?process.exit(0):process.exit(1)).catch(()=>process.exit(1))"
