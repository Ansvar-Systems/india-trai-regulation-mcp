...
function inferFrameworkId(doc: FetchedDocument): string {
  ...
  if (fn.includes("qos") || fn.includes("quality")) return "trai-qos";
  if (fn.includes("mnp")) return "trai-mnp";
  if (fn.includes("tcccpr") || fn.includes("commercialcommunication")) return "trai-tcccpr";
  if (fn.includes("interconnect")) return "trai-interconnect";
  if (fn.includes("tariff")) return "trai-tariff";
  return `trai-${doc.filename.replace(/\.pdf$/i, "").toLowerCase().replace(/[^a-z0-9]/g, "-").substring(0, 40)}`;
}

function inferCircularReference(doc: FetchedDocument): string {
  // Try to extract a TRAI reference from the text (e.g. "TRAI/No./2018/XX")
  const refMatch = doc.text.match(/TRAI[\/\-][A-Z0-9\/\-]+\d+/i);
  if (refMatch) return refMatch[0]!.toUpperCase().substring(0, 60);

  // Fall back to a reference derived from the filename and category
  const year = new Date().getFullYear();
  const base = doc.filename.replace(/\.pdf$/i, "").replace(/[^a-zA-Z0-9]/g, "-").substring(0, 40);
  return `TRAI-${doc.category.substring(0, 3).toUpperCase()}-${year}-${base}`;
}
